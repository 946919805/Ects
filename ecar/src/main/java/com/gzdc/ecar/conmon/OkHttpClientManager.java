...
public class OkHttpClientManager {
    ...
    private OkHttpClientManager() {
        ...
        //cookie enabled
        mOkHttpClient.setCookieHandler(new CookieManager((java.net.CookieStore) new PersistentCookieStore(EcarApplication.getAppContext()),
    ...
    public static void postAsyn(String url, final ResultCallback callback, Object[] objects, String[] keys, Map<String, String> stringStringMap, Param... params) {
    ...
    public static void postAsynAsString(String url, Map<String, String> params, final ResultCallback callback) {
        getInstance()._postAsynAsString(url, callback, params);
    }
     ...
     * @param params
     */
    private void _postAsynAsString(String url, final ResultCallback callback, Map<String, String> params) {
        Param[] paramsArr = map2Params(params);
        Request request = buildPostRequest(url, paramsArr);
//        deliveryResult(callback, request);
        deliveryResultAsString(callback, request);
    }
...
//                    });
    ...
    private Request buildImageMultipartFormRequest(String url, File[] files,
                                                   String[] fileKeys) {
        ...
        if (files != null) {
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                fileBody = RequestBody.create(MediaType.parse(guessMimeType(fileName)), file);
                //TODO 根据文件名设置contentType
                builder.addPart(Headers.of("Content-Type:image/pjpeg", "\r\n"), fileBody);
            }
        }
    ...
    private void deliveryResultAsString(final ResultCallback callback, Request request) {
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(final Request request, final IOException e) {
                sendFailedStringCallback(request, e, callback);
